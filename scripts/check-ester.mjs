...
// Struttura
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%ester%");
...
// Libreria
console.log("\n── LIBRERIA ──");
const kws = ["mountain climber","overhead lunge","overhead.*lunge","affondi.*overhead","clean","power clean","plank.*shoulder","shoulder.*tap","plank shoulder","side plank.*abd","chin up assist","inverted row","barbell inverted","leg extention","leg extension","v-up","v up","hollow hold","suitcase"];
for (const kw of kws) {
  const {data} = await s.from("exercise_library").select("name,category,subcategory").ilike("name",`%${kw.replace(/\.\*/g,"%")}%`).order("name");
  if (data?.length) console.log(`OK "${kw}": ${data.map(e=>e.name).slice(0,3).join(" | ")}`);
