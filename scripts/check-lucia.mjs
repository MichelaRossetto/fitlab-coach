...
// Struttura
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%lucia%").ilike("surname","%pieri%");
...
// Libreria
console.log("\n── LIBRERIA ──");
const kws = ["thoracic rotation","thoracic","hammer curl","french press","skull crusher","hamstring stretch","child pose","bird dog","arnold press","ring row","push up.*panca","shoulder dislocation","tapis roulant","skill mill","assault bike","rower","lat machine.*tri","leg curl","leg extention","leg extension","db step up","box step up","farmer carry","dead bug","plank"];
for (const kw of kws) {
