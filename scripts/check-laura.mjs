...
// Struttura
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%laura%").ilike("surname","%bincoletto%");
...
console.log("\n── LIBRERIA ──");
const kws = [
  "hip cars","hip car","landmine press","half kneeling landmine","landmine thruster",
  "db thruster","toe touch","side plank reach","fitball plank","plank.*fitball",
  "good morning","scapular push","scap push","step down","step up",
  "landmine squat","reverse lunge","walking lunge","pallof","suitcase carry",
  "dead bug","v up","sit up","plank shoulder"
];
for (const kw of kws) {
  const {data} = await s.from("exercise_library").select("name,category,subcategory").ilike("name",`%${kw.replace(/\.\*/g,"%")}%`).order("name").limit(3);
