...
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%alessandra%").ilike("surname","%rossetto%");
...
console.log("\n── LIBRERIA ──");
const kws = [
  "rematore.*manubri","one arm.*row","db row","single arm",
  "reverse lunge.*kb","kb.*reverse lunge","front rack.*lunge",
  "push up facilitati","push up ginocch",
  "half burpee","1/2 burpee","burpee sprint",
  "crunch","crunch obliq","bicycle crunch",
  "knee to chest","pullover.*manubrio","db pull over",
  "alzate laterali","lateral raise",
  "thoracic rotation","shoulder cars","hip opener","affondo dinamico",
  "goblet squat","front squat","sumo deadlift","box step up",
  "american swing","kb american","bulgarian split",
  "wall ball","sit up","plank shoulder","shoulder.*touch",
  "db bench press","lat machine"
];
for (const kw of kws) {
