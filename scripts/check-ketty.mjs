...
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%ketty%").ilike("surname","%perteg%");
...
console.log("\n── LIBRERIA ──");
const kws = [
  "hip circle","pallof hold","bird dog.*elastico","standing abduction",
  "split stance rdl","split stance","sumo.*high pull","rear delt",
  "rocking bacino","pelvic tilt","pelvic","fitball hip","landmine thruster",
  "lat machine.*inv","presa inversa","rematore.*inv","underhand row",
  "hang snatch","db hang","burpee.*box","push up.*box","affondo.*din",
  "affondo.*front","affondo.*post","goblet","z press","child","fitball breathing",
  "kb row","kb.*row.*lato","ski erg","skill mill","half kneeling.*press",
  "landmine press","respirazione","cat cow","monster walk"
];
for (const kw of kws) {
