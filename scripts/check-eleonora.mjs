...
// Struttura
const {data:clients} = await s.from("clients").select("id,name,surname").ilike("name","%eleonora%").ilike("surname","%rossi%");
...
for (const m of months||[]) {
  ...
  console.log(`\n${m.label} (${m.id}) notes="${m.notes??'null'}":`);
...
// Libreria - esercizi nuovi
console.log("\n── LIBRERIA ──");
const kws = [
  "kipping","pull up kipping","rock up","turkish sit","kb walking lunge","walking lunge",
  "clean and press","db clean","push up down dog","down dog",
  "double kb row","kb hollow","lateral burpee","burpee.*over","affondi avanti",
  "kb push press","kb front squat","kb halo","halo",
  "active hang","knee to chest","flutter kick","box jump",
  "slamball.*shoulder","slam ball.*shoulder","kb single leg","single leg rdl",
  "parallel.*leg","parallele","fitball rollout","kb thruster",
  "overhead lunge","db overhead lunge","sumo.*high pull","high pull",
  "z press","handstand","sled push","sled",
  "dip.*box","dip.*panca","russian twist.*press",
  "devil press","star plank","hollow hold","toes to bar","renegade row"
];
for (const kw of kws) {
